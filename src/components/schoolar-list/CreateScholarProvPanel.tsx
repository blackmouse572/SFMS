...
export const CreateScholarProvSchema = z.object({
...
export type CreateScholarProvSchema = z.infer<typeof CreateScholarProvSchema>;

type CreateScholarPanelProps = {
  onSubmit: (data: CreateScholarProvSchema, form: UseFormReturn<CreateScholarProvSchema>) => Promise<void>;
  defaultValues?: Partial<CreateScholarProvSchema>;
} & VariantProps &
  Omit<DialogProps, 'children'>;

function CreateScholarProvPanel(props: CreateScholarPanelProps) {
  ...
  const form = useForm<CreateScholarProvSchema>({
    resolver: zodResolver(CreateScholarProvSchema),
      ...
      <SheetContent size="3xl" className="flex h-full flex-col gap-4 overflow-y-auto">
        <Form {...form}>
          <SheetHeader>
            ...
            {!user?.provider && (
              <SheetDescription className="text-danger-500">Tài khoản không thể tạo, vui lòng sử dụng tài khoản nhà cung cấp</SheetDescription>
            )}
          </SheetHeader>
          <fieldset disabled={!user?.provider} className="disabled:opacity-60">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <TagInput
                          styleClasses={{
                            input:
                              'placeholder:text-caption rounded-none focus:ring-0 focus-visible:ring-0 focus-visible:border-0 focus:outline-none',
