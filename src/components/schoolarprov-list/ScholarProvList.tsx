import { useSchoolarProvsShip } from '@components/schoolarprov-list/useSchoolarProvsShip';
...
type SchoolarListProps = {
  filter: Partial<Pick<SchoolarShip, 'name'>>;
  providerId: string;
};
function SchoolarProvList({ filter, providerId }: SchoolarListProps) {
  const { isLoading, data } = useSchoolarProvsShip({
    filter: {
      ...filter,
      provider: providerId,
    },
  ...
  const renderItem = useCallback(
    (s: SchoolarShip) => {
      return (
        <Link to={`/nha-cung-cap/${providerId}/hoc-bong/${s._id}`} key={s._id}>
