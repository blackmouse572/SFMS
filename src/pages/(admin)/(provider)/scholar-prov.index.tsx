...
import { useEditScholarProvShip } from '@components/schoolarprov-list/useEditScholarProvShip';
...
function AdminScholarProv() {
  ...
  const user = useUser();
  const { isLoading, data, hasNextPage, isFetchingNextPage, fetchNextPage } = useSchoolarProvsShip({
    filter: {
      ...filter,
      provider: user?.provider,
    },
  });
  const { mutateAsync: create } = useCreateScholarProvShip();
  const { mutateAsync: deleteById } = useDeleteScholarProvship();
  const { mutateAsync: edit } = useEditScholarProvShip();
...
export default AdminScholarProv;
